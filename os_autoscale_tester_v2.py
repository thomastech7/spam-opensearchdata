...
def main():
    parser = argparse.ArgumentParser(
        description="OpenSearch Load Generator with Autoscaling Resilience"
    )
    parser.add_argument(
        "--scheme",
        choices=["http", "https"],
    ...
    parser.add_argument(
        ...
        type=int,
        default=500,
        help="Number of logs to insert per bulk request",
    )
    parser.add_argument(
        "--delay",
        type=float,
        default=0.5,
        help="Delay (in seconds) between bulk requests",
    )
    parser.add_argument(
        "--size-check-interval",
        type=int,
        default=20,
        help="Check index size every N batches",
    )
    parser.add_argument(
    ...
    while True:
        try:
            ...
            print(
                f"[{now_str}] Inserted: {total_inserted} docs | Accurate Size: {current_size_mb} MB",
                end="\r",
            )
            ...
            error_msg = (
                f"Connection dropped (Autoscaling/Failover likely). Error: {err}"
            )
            print(f"\n\n[!] {error_msg}")
            print(
                "[*] Yielding for 10 seconds to allow OpenSearch cluster to stabilize..."
            )
            ...
            stop_msg = (
                f"Load test stopped by user. Total documents indexed: {total_inserted}"
            )
            print(f"\n\n{stop_msg}")
